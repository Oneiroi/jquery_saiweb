...
for(i=0;i<acount;i++){time+=opts.delay;if(i==alim){setTimeout(function(){console.log('reveal');tout+=opts.text.charAt(walk);elem.html(tout);elem.html(tout)
walk++;},time);}else if(i<alim){setTimeout(function(){console.log('anim');rand=Math.floor(Math.random()*slen);tmp=tout+srand.charAt(rand);elem.html(tmp);},time);}}
count++;}}
